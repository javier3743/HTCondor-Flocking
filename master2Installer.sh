...
cat << EOT >> /etc/hosts
$(/sbin/ifconfig enp0s8 | grep -i mask | awk '{print $2}'| cut -f2 -d:) $(hostname).cloud.univalle.edu.co $(hostname)
...
post-up route add -host 172.22.52.1 gw 172.25.52.1
post-up route add -host 172.22.52.18 gw 172.25.52.1
EOT
...
TCP_FORWARDING_HOST = 172.25.52.1
# Node IP inside NAT/IP del nodo en el NAT
PRIVATE_NETWORK_INTERFACE = 172.25.52.18
PRIVATE_NETWORK_NAME = cloud.univalle.edu.co
FLOCK_TO = master1.ciat.cgiar.org
...
HOSTALLOW_NEGOTIATOR_SCHEDD = $(FLOCK_NEGOTIATOR_HOSTS),$(FLOCK_FROM),$(CONDOR_HOST)
